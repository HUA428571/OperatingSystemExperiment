...
int find_max(int a[], int BLOCK_SIZE) {//返回数组a中最大元素的地址
	int temp = 0;
...
int find_min(int a[], int BLOCK_SIZE) {//返回数组a中最小元素的地址
	int temp = 0;
	for (int i = 1; i < BLOCK_SIZE; i++) {
		if (a[i] < a[temp]) temp = i;
	}
...
int CLOCK(int BLOCK_SIZE, int page[]) {
	...
	while (t < PAGE_NUM) {
		if (scan_block_position(page[t], block, BLOCK_SIZE) != -1) {
			...
			if (block[temp % BLOCK_SIZE] == -1) {//如果物理块有空块，装入页面
				block[temp % BLOCK_SIZE] = page[t];
...
int CLOCK_advanced(int BLOCK_SIZE, int page[]) {
	...
	while (t < PAGE_NUM) {
		...
		if (scan_block_position(page[t], block, BLOCK_SIZE) != -1) {
			...
			if (block[temp % BLOCK_SIZE] == -1) {//如果有空闲的物理块，装入页面
				block[temp % BLOCK_SIZE] = page[t];
				accessed[temp % BLOCK_SIZE] = 1;
				count++;
				temp++;
