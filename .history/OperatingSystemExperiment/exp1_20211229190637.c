...
int CLOCK(int BLOCK_SIZE, int page[])
{
	...
	while (t < PAGE_NUM)
	{
		...
		else
		{
			...
			{
				while (1)
				{ //指针顺序下移，寻找第一个访问位为0的页面，将其置换。扫描过程中将扫描过的页面访问位置0。
					if (accessed[temp % BLOCK_SIZE] == 0)
						break;
					if (accessed[temp % BLOCK_SIZE] == 1)
					{
						accessed[temp % BLOCK_SIZE] = 0;
						temp++;
					}
				}
...
int CLOCK_advanced(int BLOCK_SIZE, int page[])
{
	...
	while (t < PAGE_NUM)
	{
		...
		if (block[random_Num] != -1)
			modified[random_Num] = 1; //如果随机出的物理块号中内容不为空，将其修改位置1
		//模拟系统修改结束
		flag = 1; //从第一轮扫描开始
		if (scan_block_position(page[t], block, BLOCK_SIZE) != -1)
		{
			...
			{ //置换页面
				while (flag != 0)
				{
					...
					{ //第一轮扫描，找1类页面
						for (int i = 0; i < BLOCK_SIZE; i++)
						{
							if (accessed[temp % BLOCK_SIZE] == 0 && modified[temp % BLOCK_SIZE] == 0)
							{
								flag = 0;
								break;
							}		//如果找到1类页面，退出循环，进行替换
						...
						if (flag != 0)
							flag = 2; //如果没找到，开始第二轮扫描
					}
					if (flag == 2)
					{ //第二轮扫描，找2类页面
						for (int i = 0; i < BLOCK_SIZE; i++)
						{
							if (accessed[temp % BLOCK_SIZE] == 0 && modified[temp % BLOCK_SIZE] == 1)
							{
								flag = 0;
								break;
							}								 //如果找到2类页面，退出循环，进行替换
