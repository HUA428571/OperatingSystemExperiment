...
int scan_block_position(int a, int block[], int PHYSICAL_BLOCK_NUM)
{ //搜索页面a是否在物理块中,并返回位置
...
//最佳置换算法
int OPT(int list[ARRAY_LIST_LENGTH])
{
	...
	while (t < ARRAY_LIST_LENGTH)
	{
		...
		else
		{
			...
			{
				int position[PHYSICAL_BLOCK_NUM]; //置换算法，寻找未来最久未使用的页面，将其置换
				for (int i = 0; i < PHYSICAL_BLOCK_NUM; i++)
					position[i] = scan_page_position(block[i], page);
				int temp = max(position, PHYSICAL_BLOCK_NUM);
