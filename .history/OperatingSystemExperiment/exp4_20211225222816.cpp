...
#define MAX_TIME_START 240      //最长作业开始时间
#define max 50
...
void creat(int n) //创建PCB
{
	...
	if (in2) {
		...
		{
			fscanf(in2, "\n");
			fscanf(in2, "%d\t", &pcb[i].name);
			fscanf(in2, "%d\t", &pcb[i].time_arrive);
			fscanf(in2, "%d\t", &pcb[i].time_use);
			fscanf(in2, "%d", &pcb[i].priority);
		}
		fclose(in2);
	}
	else {
	...
	for (i = 1; i <= n; i++)
	{
		printf("%d\t%d\t%d\t%d\n", pcb[i].name, pcb[i].time_arrive, pcb[i].time_use, pcb[i].priority);
	}
}
