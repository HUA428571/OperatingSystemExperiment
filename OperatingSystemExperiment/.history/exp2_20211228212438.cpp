...
void init_first_allocate(int process_resource_0[][SIZE_RESOURCE_TYPE], int n)
{
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < SIZE_RESOURCE_TYPE; j++)
		{
			process_resource_0[i][j] =
				(rand() %
					 (int)(SIZE_MAX_REASOURCE_NEED *
						   (MULTI_MAX_FIRST_ALLOCATE - MULTI_MIN_FIRST_ALLOCATE) / 5) +
				 SIZE_MAX_REASOURCE_NEED * MULTI_MIN_FIRST_ALLOCATE / 5 + 1) *
				5;
		}
	}
}
...
int main()
{
  ...
  init(process_resource_need,n);
    init_first_allocate(process_resource_0,n);
